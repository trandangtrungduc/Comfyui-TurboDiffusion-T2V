...
class WanModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # time embeddings
        # NOTE: When running with CPU offload / quantized weights, the model's Linear weights may
        # be bf16 while we previously forced the sinusoidal embedding to fp32 via `.float()`.
        # `F.linear` requires matching dtypes, so we always cast the embedding to the
        # `time_embedding` parameter dtype/device here.
