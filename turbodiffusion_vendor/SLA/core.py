...
qattn = None
fused = None
get_vanilla_qk_quant = None
block_map_lut_triton = None

# Try importing from SpargeAttn (spas_sage_attn) first
# SpargeAttn: https://github.com/thu-ml/SpargeAttn (package name: spas_sage_attn)
try:
    ...
    # Fallback: Try using SageAttention (sageattention) with wrapper functions
    # SageAttention: https://github.com/thu-ml/SageAttention (package name: sageattention)
    try:
        import torch
        if not torch.cuda.is_available():
            raise ImportError("CUDA not available")
        
        # Import SageAttention modules based on GPU architecture
        device_cap = torch.cuda.get_device_capability(0)
        arch = f"sm{device_cap[0]}{device_cap[1]}"
        
        # Try to import qattn module
        qattn = None
        for arch_name in ["sm90", "sm89", "sm80"]:
            try:
                if arch_name == "sm90":
                    from sageattention import _qattn_sm90 as qattn
                elif arch_name == "sm89":
                    from sageattention import _qattn_sm89 as qattn
                elif arch_name == "sm80":
                    from sageattention import _qattn_sm80 as qattn
                break
            except ImportError:
                continue
        
        if qattn is None:
            raise ImportError("Could not import qattn from sageattention")
        
        # Import fused module
        from sageattention import _fused as fused
        
        # Create wrapper functions for SageAttention
        from sageattention.quant import per_block_int8
        
        def get_vanilla_qk_quant(q, k, km, BLKQ, BLKK):
            """Wrapper for SageAttention's per_block_int8 to match SpargeAttn API."""
            q_int8, q_scale, k_int8, k_scale = per_block_int8(
                q, k, km=km, BLKQ=BLKQ, BLKK=BLKK, tensor_layout="HND"
            )
            return q_int8, q_scale, k_int8, k_scale
        
        # block_map_lut_triton: convert sparse_map to LUT format
        # Pure PyTorch implementation (compatible with SageAttention)
        def block_map_lut_triton(sparse_map):
            """
            Convert sparse_map to LUT format for SageAttention.
            
            Args:
                sparse_map: (B, H, Q_blocks, K_blocks) int8 tensor with 1s for selected blocks
            
            Returns:
                lut: (B, H, Q_blocks, max_blocks) int32 tensor with K block indices
                valid_block_num: (B, H, Q_blocks) int32 tensor with count of valid blocks per query
            """
            B, H, Q_blocks, K_blocks = sparse_map.shape
            max_blocks = 256
            lut = torch.zeros((B, H, Q_blocks, max_blocks), dtype=torch.int32, device=sparse_map.device)
            valid_block_num = torch.zeros((B, H, Q_blocks), dtype=torch.int32, device=sparse_map.device)
            
            for b in range(B):
                for h in range(H):
                    for q in range(Q_blocks):
                        # Find non-zero blocks for this query block
                        k_indices = torch.nonzero(sparse_map[b, h, q] > 0, as_tuple=False).squeeze(-1)
                        if len(k_indices) > 0:
                            k_indices = k_indices.to(torch.int32)
                            count = min(len(k_indices), max_blocks)
                            lut[b, h, q, :count] = k_indices[:count]
                            valid_block_num[b, h, q] = count
...
class SageSparseLinearAttention(nn.Module):
    def __init__(self, head_dim, topk, feature_map='softmax', use_bf16=True, tie_feature_map_qk=True):
        ...
        assert SAGESLA_ENABLED, (
            "SageSLA requires SpargeAttn package (not SageAttention!).\n"
            "Install it with: pip install git+https://github.com/thu-ml/SpargeAttn.git\n"
            "Note: SpargeAttn (spas_sage_attn) is different from SageAttention (sageattention).\n"
            "Or use 'sla' attention type instead (does not require SpargeAttn)."
        )
