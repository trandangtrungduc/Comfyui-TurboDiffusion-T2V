...
# Import SageSLA only if available (requires SpargeAttn)
SageSLA = None
SAGESLA_ENABLED = False
try:
    from SLA.core import SAGESLA_ENABLED, SageSparseLinearAttention
    if SAGESLA_ENABLED:
        SageSLA = SageSparseLinearAttention
...
def replace_attention(
    ...
    # Check if SageSLA is available when using sagesla
    if attention_type == "sagesla":
        if not SAGESLA_ENABLED or SageSLA is None:
            raise RuntimeError(
